...
const WritingSamplesPage = () => {
  ...
  const filterSamples = (task: string, questionType: string) => {
    ...
    // Filter by task
    if (task === 'task1') {
      filtered = filtered.filter(sample => sample.fields.taskType === 'Task 1');
    } else if (task === 'task2') {
      filtered = filtered.filter(sample => sample.fields.taskType === 'Task 2');
    }

    // Filter by question type
    if (questionType !== 'all') {
      filtered = filtered.filter(sample => sample.fields.questionType === questionType);
    }
          ...
          {selectedTask !== 'all' && (
            <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
              <h3 className="text-xl font-bold text-gray-800 mb-4 text-center">
                {selectedTask === 'task1' ? 'Task 1 Question Types' : 'Task 2 Question Types'}
              </h3>
              <div className="flex flex-wrap gap-3 justify-center">
                ...
                {(selectedTask === 'task1' ? task1QuestionTypes : task2QuestionTypes).map((questionType) => (
                  <button
                    key={questionType}
          ...
          <div className="text-center py-20">
            ...
            <h3 className="text-2xl font-semibold text-gray-800 mb-2">
              {selectedTask === 'all' 
                ...
                : selectedQuestionType === 'all'
                  ? `No ${selectedTask === 'task1' ? 'Task 1' : 'Task 2'} samples found`
                  : `No ${selectedQuestionType} samples found`
              }
            </h3>
            <p className="text-gray-500 text-lg">Check back later for new samples</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredSamples.map((sample) => (
              <div key={sample.sys.id} className="group bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 overflow-hidden border border-gray-100">
                {/* Image */}
                {sample.fields.image && (
