...
const MultipleMCQGroupForm = ({ questions, onUpdate }: MultipleMCQGroupFormProps) => {
    const [localQuestions, setLocalQuestions] = useState<MultipleMCQItem[]>(questions);
    ...
    const updateQuestionField = (index: number, field: keyof MultipleMCQItem, value: any) => {
    ...
    const updateCorrectMapping = (questionIndex: number, mappingIndex: number, value: string) => {
        const updatedQuestion = { ...localQuestions[questionIndex] };
    ...
    const removeCorrectMapping = (questionIndex: number, mappingIndex: number) => {
        const updatedQuestion = { ...localQuestions[questionIndex] };
            ...
            {localQuestions.map((question, questionIndex) => (
                <div key={questionIndex} className="card bg-base-100 p-4 border">
                    <div className="flex justify-between items-center mb-4">
                        <h4 className="text-lg font-medium">Multiple MCQ Question {questionIndex + 1}</h4>
                        <button
                            type="button"
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="form-control">
                            <label className="label">
                                <span className="label-text font-semibold">Question Numbers</span>
                            </label>
                                ...
                                {question.question_numbers.map((num, numIndex) => (
                                    <input
                                        ...
                                        onChange={(e) => {
                                            ...
                                            updateQuestionField(questionIndex, 'question_numbers', newNumbers);
                                        }}
                                    />
                                ))}
                                <button
                                    type="button"
                                    onClick={() => {
                                        const newNumbers = [...question.question_numbers, question.question_numbers.length + 1];
                                        updateQuestionField(questionIndex, 'question_numbers', newNumbers);
                                    }}
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="form-control">
                                <label className="label">
                                    <span className="label-text font-semibold">Min Selection</span>
                                </label>
                                <input
                                    type="number"
                                    className="input input-bordered"
                                    value={question.min_selection}
                                    onChange={(e) => updateQuestionField(questionIndex, 'min_selection', parseInt(e.target.value))}
                                    min={1}
                                />
                            </div>
                            <div className="form-control">
                                <label className="label">
                                    <span className="label-text font-semibold">Max Selection</span>
                                </label>
                                <input
                                    type="number"
                                    className="input input-bordered"
                                    value={question.max_selection}
                                    onChange={(e) => updateQuestionField(questionIndex, 'max_selection', parseInt(e.target.value))}
                                    min={1}
                                />
                            </div>
                        </div>
                        ...
                        <div className="form-control">
                            <label className="label">
                                <span className="label-text font-semibold">Correct Answers (in order of question numbers)</span>
                            </label>
                            <div className="space-y-2">
                                {question.correct_mapping.map((mapping, mappingIndex) => (
                                    <div key={mappingIndex} className="flex gap-2 items-center">
                                        <span className="font-medium">Q{question.question_numbers[mappingIndex]}:</span>
                                        <select
                                            className="select select-bordered"
                                            value={mapping}
                                            onChange={(e) => updateCorrectMapping(questionIndex, mappingIndex, e.target.value)}
                                        >
                                        ...
                                        <button
                                            type="button"
                                            onClick={() => removeCorrectMapping(questionIndex, mappingIndex)}
                                            className="btn btn-error btn-sm"
                                        >
                                            Remove
                                        </button>
